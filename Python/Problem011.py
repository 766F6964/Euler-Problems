...
def largest_grid_product():
    ...
    for i in range(0, 20):
        for r in range(0, 20 - 4):
            prod = grid[i][r] * grid[i][r + 1] * grid[i][r + 2] * grid[i][r + 3]
            if prod > max_prod:
                max_prod = prod
        for r in range(0, 20 - 4):
            prod = grid[r][i] * grid[r + 1][i] * grid[r + 2][i] * grid[r + 3][i]
            if prod > max_prod:
                max_prod = prod

    for i in range(0, 20 - 4):
        for r in range(0, 20 - 4):
            prod = grid[i][r] * grid[i + 1][r + 1] * grid[i + 2][r + 2] * grid[i + 3][r + 3]
            if prod > max_prod:
                max_prod = prod
        for r in range(20 - 1, 4, -1):
            prod = grid[r][i] * grid[r - 1][i + 1] * grid[r - 2][i + 2] * grid[r - 3][i + 3]
            if prod > max_prod:
                max_prod = prod
